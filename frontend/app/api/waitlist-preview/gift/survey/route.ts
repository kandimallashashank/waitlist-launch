...
/** Tag when the user dismisses without fields (one-time flow). */
const DISMISSED_TAG = "pilot_gift_survey_dismissed";

async function requireGiftSurveyContext(req: Request) {
...
/**
 * Whether this waitlist email already submitted or dismissed the gift pilot survey.
 */
export async function GET(req: Request) {
  const ctx = await requireGiftSurveyContext(req);
  if ("error" in ctx) return ctx.error;

  const { data, error } = await ctx.supabase
    .from("waitlist_gift_survey")
    .select("id")
    .eq("email", ctx.email)
    .limit(1)
    .maybeSingle();
...
/**
 * Post–gift-finder pilot feedback. At most one row per email; repeats return ok without inserting.
 */
export async function POST(req: Request) {
  try {
    ...
    const { error } = await supabase.from("waitlist_gift_survey").insert({
      email,
