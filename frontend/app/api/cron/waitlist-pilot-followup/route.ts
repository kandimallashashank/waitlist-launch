...
import { runPilotFollowupCron, type PilotFollowupPendingRow } from "@/lib/waitlist/pilotFollowupCron";
...
export async function GET(req: Request) {
  ...
  const { data: rows, error } = await supabase
    ...
    .select("email, quiz_completed_at, layering_first_success_at, followup_email_sent_at, updated_at")
  ...
  const batchSize = Number.parseInt(
    process.env.WAITLIST_PILOT_FOLLOWUP_BATCH_SIZE ?? "25",
    10,
  );
  const nowMs = Date.now();
  const pendingRows = (rows ?? []) as PilotFollowupPendingRow[];

  const stats = await runPilotFollowupCron({
    rows: pendingRows,
    nowMs,
    delayMs,
    batchSize: Number.isFinite(batchSize) ? Math.max(batchSize, 1) : 25,
    claimRow: async (row, claimedAtIso) => {
      const query = supabase
        .from("waitlist_pilot_followup")
        .update({ updated_at: claimedAtIso })
        .eq("email", row.email)
        .is("followup_email_sent_at", null);
      const filteredQuery = row.updated_at
        ? query.eq("updated_at", row.updated_at)
        : query.is("updated_at", null);
      const { data, error: claimError } = await filteredQuery
        .select("email")
        .maybeSingle();

      if (claimError) {
        console.error("Failed to claim pilot follow-up row", row.email, claimError);
        return false;
      }
    ...
    sendRow: async (row, idempotencyKey) => {
      ...
      const { data: layerRows } = await supabase
        ...
        .select(
          "summary_snippet, harmony_score, harmony_label, fragrance_names, fragrance_images",
        )
        .eq("email", email)
        .eq("success", true)
    ...
    markSent: async (row, sentAtIso) => {
      const { data, error: markError } = await supabase
        .from("waitlist_pilot_followup")
        .update({
          followup_email_sent_at: sentAtIso,
      ...
      if (markError) {
        console.error("Failed to mark pilot email sent", row.email, markError);
        return false;
      }
